...
import { ALGORITHMS, DEFAULT_COLOR, COMPARE_COLOR, SWAP_COLOR, SORTED_COLOR, BASE_ANIMATION_SPEED_MS } from './constants';

const App: React.FC = () => {
    const [arraySize, setArraySize] = useState<number>(50);
    const [bars, setBars] = useState<Bar[]>([]);
    const [selectedAlgorithm, setSelectedAlgorithm] = useState<Algorithm>('Bubble Sort');
            ...
            const timeoutId = setTimeout(() => {
                setBars(prevBars => {
                    ...
                    if (type === 'compare') {
                        ...
                        if (newBars[idx1]) newBars[idx1] = { ...newBars[idx1], color: COMPARE_COLOR };
                        if (idx2 < newBars.length && newBars[idx2]) newBars[idx2] = { ...newBars[idx2], color: COMPARE_COLOR };
                    } else if (type === 'revert') {
                        const [idx1, idx2] = indices;
                        if (newBars[idx1]) newBars[idx1] = { ...newBars[idx1], color: DEFAULT_COLOR };
                         if (idx2 < newBars.length && newBars[idx2]) newBars[idx2] = { ...newBars[idx2], color: DEFAULT_COLOR };
                    } else if (type === 'swap') {
                        const [idx1, idx2] = indices;
                        const [val1, val2] = values!;
                        if (newBars[idx1]) newBars[idx1] = { value: val1, color: SWAP_COLOR };
                        if (newBars[idx2]) newBars[idx2] = { value: val2, color: SWAP_COLOR };
    ...
    const finishAnimation = () => {
        const delay = (BASE_ANIMATION_SPEED_MS / speed) / 2;
